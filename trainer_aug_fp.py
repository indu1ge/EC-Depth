...
class Trainer:
    def __init__(self, options):
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext, augmix=self.opt.augmix, aug_fp=self.opt.aug_fp)
        ...
        val_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, is_train=False, img_ext=img_ext, augmix=self.opt.augmix, aug_fp=self.opt.aug_fp)
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.num_epochs):
            ...
            if (self.epoch + 1) % self.opt.save_frequency == 0 and self.epoch>14:
                self.save_model()
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs_fp = {}
            if self.opt.augmix:
                images_all = torch.cat((inputs["color_aug_weak", 0, 0], 
                                    inputs["color_aug", 0, 0],
                                    inputs["color_aug1", 0, 0]), 0)
                ...
                features = self.models["encoder"](inputs["color_aug_weak", 0, 0])
        ...
        losses = self.compute_losses(inputs, outputs, outputs_aug1, outputs_aug2, outputs_fp)
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type == "shared":
                ...
                pose_feats = {f_i: inputs["color_aug_weak", f_i, 0] for f_i in self.opt.frame_ids}
    ...
    def compute_reprojection_loss(self, pred, target):
        ...
        l1_loss = abs_diff.mean(1, True)    # B x 1 x H x W
    ...
    def compute_losses(self, inputs, outputs, outputs_aug=None, outputs_aug1=None, outputs_fp=None):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.augmix or self.opt.aug_fp:
                ...
                # disp_mix = torch.clamp((disp + disp_aug + disp_aug1) / 3., 1e-7, 1)
                loss += 0.0005 * (F.kl_div(disp_mix, disp, reduction='batchmean') 
                        + F.kl_div(disp_mix, disp_aug, reduction='batchmean')
                        + F.kl_div(disp_mix, disp_aug1, reduction='batchmean')) / 3.
                # loss += 0.0005 * (((torch.log(disp_mix) - torch.log(disp))
                #                   + torch.log(disp_mix) - torch.log(disp_aug)
                #                   + torch.log(disp_mix) - torch.log(disp_aug1)) / 3.).mean()

            # fp
            if self.opt.fp or self.opt.aug_fp:
                disp_fg = outputs_fp[("disp", scale)]
                disp_fg_avg = torch.clamp((disp_fg + disp) / 2, 1e-7, 1).log()
                # disp_fg_avg = torch.clamp((disp_fg + disp) / 2, 1e-7, 1)
                loss += 0.0005* (F.kl_div(disp_fg_avg, disp, reduction='batchmean')
                               + F.kl_div(disp_fg_avg, disp_fg, reduction='batchmean')) / 2
                # loss += 0.0005 * (((torch.log(disp_fg_avg) - torch.log(disp))
                #                   + torch.log(disp_fg_avg) - torch.log(disp_fg)) / 2.).mean()
