...
class Trainer:
    def __init__(self, options):
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, is_train=True, img_ext=img_ext, augmix=self.opt.augmix,
            aug_fp=self.opt.aug_fp, severity=self.opt.severity)
        ...
        val_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, is_train=False, img_ext=img_ext, severity=self.opt.severity)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.augmix:
            ...
            # student
            student_features = self.student_models["encoder"](inputs["color_aug_weak", 0, 0])
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type in ["separate_resnet", "posecnn"]:
                pose_inputs = torch.cat(
                    [inputs[("color_aug_weak", i, 0)] for i in self.opt.frame_ids if i != "s"], 1)
            ...
            for s in self.opt.scales:
                ...
                if s == 0:
                    for frame_id in self.opt.frame_ids:
                        ...
                        if frame_id == 0:
                            writer.add_image(
                                "color_aug_weak_{}_{}/{}".format(frame_id, s, j),
                                inputs[("color_aug_weak", frame_id, s)][j].data, self.step)
                            if self.opt.augmix or self.opt.aug_fp:
                                writer.add_image(
                                    "color_aug_{}_{}/{}".format(frame_id, s, j),
                                    inputs[("color_aug", frame_id, s)][j].data, self.step)
