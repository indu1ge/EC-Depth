            ...
            for bl_intensity in blur_intensity:
                ...
                with torch.no_grad():
                    ...
                    for data in dataloader:
                        ...
                        #     # output_path = os.path.join(opt.eval_out_dir, bl_category, bl_intensity,filenames[i+j].split(' ')[1]+img_ext)
                        #     # outdir = os.path.join(opt.eval_out_dir, bl_category, bl_intensity)
                        #     output_path = os.path.join(opt.eval_out_dir, filenames[i+j].split(' ')[1]+img_ext)
                        #     outdir = opt.eval_out_dir
                        #     if not os.path.exists(outdir):
                        #         os.makedirs(outdir)
                        #     cv2.imwrite(output_path, colormap(pred_disp, normalize=False)[j,:,:,::-1]*256)
